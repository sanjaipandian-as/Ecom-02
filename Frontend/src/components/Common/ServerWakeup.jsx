...
const ServerWakeup = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            window.removeEventListener('render-wakeup', handleWakeupEvent);
            if (progressInterval) clearInterval(progressInterval);
    ...
    return (
        <div className={`fixed bottom-6 right-6 z-9999 max-w-sm w-full bg-emerald-deep/95 backdrop-blur-md border border-gold-champagne/30 text-cream-base rounded-xl shadow-2xl p-5 transition-all duration-500 ease-in-out ${
            isClosing ? 'opacity-0 translate-y-4 scale-95' : 'opacity-100 translate-y-0 scale-100 animate-fade-in'
        }`}>
            {/* Header */}
