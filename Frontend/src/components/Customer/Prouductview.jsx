                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 xl:gap-16 mb-16 items-start">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {productImages.slice(0, 4).map((img, i) => (
                                    <div key={i} className={`overflow-hidden rounded-lg group cursor-pointer relative bg-slate-50 ${productImages.length === 1 ? 'col-span-1 md:col-span-2 aspect-[4/5] md:aspect-square' : 'aspect-square md:aspect-[4/5]'}`}>
                                        <img
                                            src={img}
                    ...
                    <div className="lg:col-span-7 xl:col-span-7 flex flex-col">
                        ...
                        <div className="mb-4 font-sans">
                            <h1 className="text-2xl xl:text-3xl text-teal-600 mb-2 font-medium capitalize">
                                {product.name.toLowerCase()}
                            </h1>
                            <p className="text-red-400 text-[13px] mb-5">
                                In Demand! 10+ Shoppers bought this in the last 30 Days
                            </p>

                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-3">
                                    <div className="flex items-center gap-1 border border-slate-200 rounded-full px-3 py-1.5 bg-white shadow-sm">
                                    ...
                                    <button className="border border-slate-200 rounded-full px-4 py-1.5 bg-white shadow-sm text-sm text-slate-600 flex items-center gap-2 hover:bg-slate-50 transition-colors">
                                        View Details <FaArrowDown className="text-[10px] text-slate-400" />
                                    </button>
                                </div>
                                <div className="flex items-center gap-3">
                                    <button onClick={handleShare} className="w-10 h-10 border border-slate-200 rounded-xl flex items-center justify-center bg-white shadow-sm text-slate-500 hover:text-teal-600 transition-colors">
                                        <FaShareAlt />
                                    </button>
                                    <button onClick={toggleWishlist} className="w-10 h-10 border border-slate-200 rounded-xl flex items-center justify-center bg-white shadow-sm text-slate-500 hover:text-red-500 transition-colors">
                                        {isInWishlist ? <FaHeart className="text-red-500" /> : <FaRegHeart />}
                                    </button>
                                </div>
                            </div>

                            <div className="bg-gradient-to-r from-purple-400 to-purple-500 text-white text-[10px] font-bold px-3 py-1.5 w-fit rounded-r-md rounded-bl-md mb-4 flex items-center gap-1 uppercase tracking-wider shadow-sm">
                                <FaStar className="text-[8px]" /> TOP RATED
                            </div>

                            <div className="flex items-end gap-3 mb-3">
                                <span className="text-3xl font-bold text-teal-600 tracking-tight">₹{sellingPrice.toLocaleString()}</span>
                                {mrp > sellingPrice && (
                            ...
                            <div className="flex items-center justify-between mb-8">
                                <div className="bg-teal-50 text-teal-600 text-[10px] font-bold px-3 py-1.5 uppercase tracking-widest">
                                    OFFER: {discount > 0 ? `${discount}% OFF` : 'SPECIAL PRICE'} ON MAKING
                                </div>
                                <div className="text-xs">
                                    <button className="text-teal-500 underline underline-offset-2 hover:text-teal-600">Notify me</button> <span className="text-slate-500">When Price Drops</span>
                                </div>
                            </div>
                        </div>

                        {/* Shipping Info */}
                        <div className="border border-slate-200 rounded-xl p-4 flex gap-4 mb-8 bg-white shadow-sm">
                            <div className="mt-1"><FaTruck className="text-slate-500 text-xl" /></div>
                            <div>
                                ...
                                <button className="text-teal-500 text-xs underline underline-offset-2 hover:text-teal-600">T&C Apply</button>
                            </div>
                        </div>

                        {/* Sizing & Selection Box */}
                        <div className="bg-white rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100 p-8 mb-10 relative overflow-hidden">
                            <h3 className="text-center font-serif text-xl text-slate-800 mb-3">Sizing & Selection</h3>
                            <div className="w-8 h-[2px] bg-teal-500 mx-auto mb-8"></div>
                            
                            <label className="block text-sm text-slate-600 mb-2">SelectChain Size</label>
                            <div className="relative mb-6">
                                <select 
                                    value={selectedSize} 
                                    onChange={(e) => setSelectedSize(e.target.value)} 
                                    className="w-full border border-slate-200 rounded-xl p-4 appearance-none text-slate-700 bg-white focus:outline-none focus:border-teal-500 transition-colors cursor-pointer text-sm"
                                >
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div className="flex items-center border border-slate-200 rounded-lg h-10 w-28 px-1">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 w-full">
                                <button
                                    ...
                                    disabled={!inStock}
                                    className={`flex-1 font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all border-2 ${
                                        isInCart 
                                            ? 'bg-teal-50/50 border-teal-200 text-teal-700 hover:bg-teal-100/50' 
                                            : 'bg-white hover:bg-teal-50/30 border-[#45C3D3] text-[#45C3D3]'
                                ...
                                <button
                                    ...
                                    disabled={!inStock}
                                    className="flex-1 bg-[#45C3D3] hover:bg-teal-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all disabled:bg-slate-300 shadow-md shadow-teal-500/10 hover:shadow-lg active:scale-[0.98]"
                                >
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 py-4">
                                ...
                                <div className="lg:col-span-4 space-y-8 pr-0 lg:pr-6 lg:border-r lg:border-slate-100">
                                    ...
                                    <div className="text-center lg:text-left space-y-2">
                                        ...
                                        <div className="flex justify-center lg:justify-start gap-1 py-1">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                <FaStar 
                                                    key={star} 
                                                    className={star <= Math.round(Number(averageRating)) ? 'text-teal-600 text-lg' : 'text-slate-200 text-lg'} 
                                                />
                                            ))}
                                        </div>
                                    ...
                                    <div className="p-5 bg-teal-50/50 border border-teal-100 flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center flex-shrink-0">
                                            <FaThumbsUp className="text-sm" />
                                        </div>
                                        <span className="text-xs font-bold text-teal-900 leading-relaxed uppercase tracking-wider">
                                ...
                                <div className="lg:col-span-8 space-y-8">
                                    ...
                                    {showReviewForm && (
                                        <div className="bg-white p-6 sm:p-8 border border-slate-200 space-y-6">
                                            <h4 className="font-bold text-black text-sm uppercase tracking-widest text-center border-b border-slate-100 pb-4">Rate & Review This Product</h4>
                                                ...
                                                <div className="flex justify-center gap-3">
                                                    {[1, 2, 3, 4, 5].map((star) => (
                                                        <button
                                                            ...
                                                            className={`text-2xl transition-transform hover:scale-110 ${star <= reviewForm.rating ? 'text-teal-600' : 'text-slate-200'}`}
                                                        >
                                                            <FaStar />
                                                        </button>
                                                    ))}
                                                    ...
                                                    {availableTags.map((tag) => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`px-3 py-2 border text-[10px] font-bold uppercase tracking-widest transition-all ${
                                                                    isSelected 
                                                ...
                                                <div key={r._id} className="py-6 border-b border-slate-100 last:border-0 hover:bg-slate-50/40 px-2 transition-colors">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
                                                        ...
                                                        <div className="space-y-2">
                                                            <div className="flex items-center gap-3 flex-wrap">
                                                                {/* Star tag badge */}
                                                                <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#F1FBF7] text-[#0A5C36] font-bold text-[11px] uppercase tracking-wide border border-[#D1F2E5]">
                                                    ...
                                                    <div className="flex items-center gap-4 text-xs font-bold text-slate-400 uppercase tracking-widest">
                                                        ...
                                                        <button 
                                                            ...
                                                            className="flex items-center gap-1 hover:text-teal-600 transition-colors text-slate-500"
                                                        >
                                        ...
                                        <div className="flex justify-center items-center gap-2 pt-6 border-t border-slate-100">
                                            ...
                                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                                                <button
                                                    ...
                                                    className={`w-9 h-9 font-semibold flex items-center justify-center transition-colors border ${
                                                        p === reviewPage 
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100 lg:hidden z-50 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
                <div className="flex gap-3 max-w-md mx-auto">
                    <button 
                        onClick={() => addToCartAction(false)} 
                        disabled={!inStock}
                        className={`flex-1 h-12 font-bold text-xs rounded-xl flex items-center justify-center gap-2 border-2 transition-all ${
                            isInCart 
                                ? 'bg-teal-50 border-teal-200 text-teal-700' 
                                : 'bg-white border-[#45C3D3] text-[#45C3D3]'
                        } disabled:border-slate-200 disabled:text-slate-400 disabled:bg-slate-50`}
                    >
                    ...
                    <button 
                        onClick={() => addToCartAction(true)} 
                        disabled={!inStock}
                        className="flex-1 h-12 bg-[#45C3D3] hover:bg-teal-500 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-md shadow-teal-500/10 active:scale-[0.98] disabled:bg-slate-300 disabled:shadow-none"
                    >
